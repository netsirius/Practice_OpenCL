// Practice3.4.cpp: define el punto de entrada de la aplicación de consola.
//
...
"   output[i * rows + j] = input[i * rows + j] + input2[i * rows + j];                                \n" \
"}                                                                      \n" \
"\n";
...
int main(void)
{
	...
	const int cols = 5;
	int count = rows * cols;
	cl_double* data = new cl_double[count];
	cl_double* data2 = new cl_double[count];
	cl_double* results = new cl_double[count];
	...
	//
	kernel = clCreateKernel(program, "addMatrix", &error);
	...
	error |= clSetKernelArg(kernel, 3, sizeof(unsigned int), &rows);
	error |= clSetKernelArg(kernel, 4, sizeof(unsigned int), &cols);
	if (error != CL_SUCCESS)
	{
